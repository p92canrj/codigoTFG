...
class NNPOM(ClassifierMixin, BaseEstimator):
    ...
    def __init__(self, epsilon_init=0.5, n_hidden=50, max_iter=500, lambda_value=0.01, class_weight=None):
    ...
    def fit(self, X, y):
        ...
        # Map y sequentially to 1...n_classes for internal logic
        ...
        if self.class_weight == "balanced":
            ...
            sample_weight = compute_sample_weight("balanced", y.ravel())
    ...
    def _nnpom_cost_function(
        self, nn_params, n_features, n_hidden, n_classes, X, Y, lambda_value, sample_weight
    ):
        ...
        cross_entropy_terms = -np.log(out[np.where(Y == 1)])
        weighted_ce = cross_entropy_terms * sample_weight.flatten()

        J = np.sum(weighted_ce) / n_samples + lambda_value * p / (2 * n_samples)

        # Gradients (weighted)
        error_der = np.zeros(Y.shape)
        y_nz_indices = np.where(Y != 0)
        error_der[y_nz_indices] = np.divide(
            -Y[y_nz_indices], out[y_nz_indices]
        )
        error_der = error_der * sample_weight
